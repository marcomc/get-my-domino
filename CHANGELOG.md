...
- Switched `pyproject.toml` license metadata to SPDX string form to avoid the
  current setuptools deprecation warning during builds.
...
- `sync-magazine` now handles issue audiobook packaging per issue instead of
  waiting for all article audio in the whole run to finish first. Sync can now
  package an issue as soon as that issue's chapter audio is complete,
  including reusing chapter audio generated by a previous interrupted run.

### Audio

- Raised the default `audio_chunk_concurrency` from `3` to `4` after
  benchmarking chunked `say` synthesis on a real missing article. On the test
